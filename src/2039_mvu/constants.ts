...
export type 选举进程阶段 = (typeof 选举进程阶段列表)[number];
...
export const 副大臣任命方式列表 = ['正常任命', '政治分赃', '阁僚亲信安插', '事务次官推荐', '派系妥协'] as const;
export type 副大臣任命方式 = (typeof 副大臣任命方式列表)[number];

export const 副大臣对阁僚态度列表 = [
  '忠实辅佐',
  '表面配合暗中掣肘',
  '完全听命于事务次官',
  '独立行事',
...
export type 副大臣对阁僚态度 = (typeof 副大臣对阁僚态度列表)[number];

export const 副大臣对官僚态度列表 = ['代表阁僚对抗', '居中协调', '偏向官僚', '完全被官僚同化', '与官僚争权'] as const;
export type 副大臣对官僚态度 = (typeof 副大臣对官僚态度列表)[number];

// ─── 事务次官子Schema ───────────────────────────────────────────

export const 事务次官出身派系列表 = [
  '本省主流局路线',
  '大臣官房人事路线',
  '预算/主计路线',
  '法规/条约审查路线',
...
export type 事务次官出身派系 = (typeof 事务次官出身派系列表)[number];

export const 事务次官对阁僚态度列表 = ['表面服从', '消极抵抗', '架空阁僚', '主动配合', '暗中操控', '公开对抗'] as const;
export type 事务次官对阁僚态度 = (typeof 事务次官对阁僚态度列表)[number];

export const 事务次官实权程度列表 = [
  '名义管理',
  '日常运作主导',
  '政策制定主导',
  '完全掌控省厅',
...
export type 事务次官实权程度 = (typeof 事务次官实权程度列表)[number];

export const 事务次官更替方式列表 = [
  '正常退休更替',
  '内部晋升',
  '派系斗争上位',
  '被阁僚强行更换',
...
export type 事务次官更替方式 = (typeof 事务次官更替方式列表)[number];
...
export const 省厅与下属机构: Record<string, string[]> = {
  ...
  财务省: ['大臣官房', '主计局', '主税局', '关税局', '理财局', '国际局', '国税厅'],
  内阁官房: ['内阁广报官', '内阁情报官', '国家安全保障局', '内阁参事官室'],
  警察厅: ['长官官房', '生活安全局', '刑事局', '交通局', '警备局', '情报通信局', '组织犯罪对策部', '国际部'],
  国土交通省: [
    '大臣官房',
    '综合政策局',
    '国土政策局',
  ...
  国家公安委员会: ['（警察厅为其执行机构）'],
  内阁府: ['大臣官房', '赏勋局', '男女共同参画局', '冲绳振兴局'],
  宫内厅: ['长官官房', '侍从职', '东宫职', '式部职', '书陵部', '管理部'],
  金融厅: ['总务企画局', '企画市场局', '监督局', '证券取引等监视委员会', '公认会计士·监查审查会'],
  消费者厅: ['总务课', '消费者政策课', '表示对策课', '食品表示企画课'],
  儿童家庭厅: ['长官官房', '儿童政策统括官', '儿童家庭支援统括官'],
  公平交易委员会: ['事务总局', '经济取引局', '审查局'],
  内阁法制局: ['长官总务室', '第一部', '第二部', '第三部', '第四部'],
  内阁人事局: ['人事政策统括官', '公务员制度研究官'],
  人事院: ['事务总局', '公务员研修所'],
  会计检查院: ['事务总局', '检查第一部', '检查第二部', '检查第三部', '检查第四部', '检查第五部'],
  数字化厅: ['数字厅官房', '数字政策统括官', '数字社会推进统括官'],
  复兴厅: ['复兴厅官房', '复兴政策统括官', '复兴事业统括官'],
} as const;
...
// ============================================================
// 省厅事务方一把手称呼映射（仅用于前端显示，MVU变量不变）
// 日本制度：省→事務次官 / 庁→長官 / 委員会→委員長 / 院→院長or総裁
// ============================================================

/** 省厅名 → 事务方一把手的职位称呼 */
const 省厅一把手称呼: Record<string, string> = {
  // ── 省（Ministries）→ 事務次官 ──
  外务省: '事务次官',
  防卫省: '事务次官',
  经济产业省: '事务次官',
...
/** 根据省厅名获取事务方一把手的正确职位称呼（仅用于前端显示） */
export function get事务方一把手称呼(name: string): string {
  return 省厅一把手称呼[name] ?? '事务次官';
}
